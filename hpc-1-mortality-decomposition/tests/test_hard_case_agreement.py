...
from chapter1_mortality_decomposition.hard_case_agreement import (
    ...
    _resolve_default_xgb_input_root,
    _resolve_default_xgb_recalibration_root,
    run_asic_hard_case_agreement_baseline,
...
class HardCaseAgreementTests(TestCase):
    ...
    def test_default_xgb_input_root_resolution_prefers_cluster_export_predictions(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            cluster_root = (
                tmp_path
            ...
            resolved_root, resolution = _resolve_default_xgb_input_root(
                preferred_result_kind=RESULT_ROOT_KIND_CLUSTER_EXPORT,
                repo_root=tmp_path,
            )
    ...
    def test_run_asic_hard_case_agreement_baseline_writes_raw_xgboost_outputs(self) -> None:
        logistic_prediction_rows = [
            {
                "instance_id": "n1__b0__h24",
                "stay_id_global": "n1",
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            xgb_dir = logistic_root / "xgboost" / "horizon_24h"
            xgb_dir.mkdir(parents=True)
            pd.DataFrame(xgb_prediction_rows).to_csv(xgb_dir / "predictions.csv", index=False)

            result = run_asic_hard_case_agreement_baseline(
                logistic_input_root=logistic_root,
                xgb_input_root=logistic_root,
                output_dir=tmp_path / "agreement_baseline",
                horizons=[24],
            )

            self.assertEqual(result.comparator_source, "baseline")
            self.assertEqual(result.comparator_model_name, "xgboost")
            self.assertEqual(result.comparator_prefix, "xgb")
            self.assertTrue(result.artifacts.stay_level_path.exists())
            self.assertTrue(result.artifacts.horizon_summary_path.exists())
            self.assertTrue(result.artifacts.manifest_path.exists())
            ...
            self.assertEqual(
                ...
                [
                    ...
                    "logistic_hard_case_rule",
                    "xgb_model_name",
                    "xgb_predicted_probability",
                    "xgb_nonfatal_q75_threshold",
                    "xgb_hard_case_flag",
                    "xgb_instance_id",
                    "xgb_block_index",
                    "xgb_prediction_time_h",
                    "xgb_hard_case_rule",
                    "hard_case_agreement_flag",
                    "hard_case_logistic_only_flag",
                    "hard_case_xgb_only_flag",
            ...
            self.assertEqual(int(summary.at[0, "n_fatal_with_both_models_available"]), 3)
            self.assertEqual(int(summary.at[0, "n_fatal_logistic_only_available"]), 1)
            self.assertEqual(int(summary.at[0, "n_fatal_xgb_only_available"]), 1)
            self.assertEqual(int(summary.at[0, "n_fatal_dropped_unmatched"]), 2)
            self.assertEqual(int(summary.at[0, "n_logistic_hard"]), 2)
            self.assertEqual(int(summary.at[0, "n_xgb_hard"]), 2)
            self.assertEqual(int(summary.at[0, "n_both_hard"]), 1)
            self.assertEqual(int(summary.at[0, "n_logistic_only"]), 1)
            self.assertEqual(int(summary.at[0, "n_xgb_only"]), 1)
            self.assertAlmostEqual(float(summary.at[0, "jaccard_hard_case_overlap"]), 1 / 3)
